...
(function (global) {
  ...
  const CoopRoom = {
    isCoop:false, isHost:false, roomCode:'', lobby:null, client:null, remote:new Map(), opts:null, lastStateSend:0, lastEnemySend:0, runSeed:0,
    ...
    bind(c) {
      ...
      c.addEventListener('close', () => { this.isCoop = false; this.opts?.onLobby?.(this.lobby); });
    },
    onMessage(msg) {
      const M = NetProtocol.MSG;
      if (msg.type === M.ROOM_STATE) { this.lobby = msg.room; this.isHost = msg.room?.hostPlayerId === this.profile().id; this.opts?.onLobby?.(this.lobby); this.syncRemotePlayers(); }
      ...
      else if (msg.type === M.ENEMY_STATE && !this.isHost) this.opts?.onEnemyStateNet?.(msg.enemies || []);
      else if (msg.type === M.HIT && this.isHost && msg.playerId !== this.profile().id) this.opts?.onHitNet?.(msg);
      else if (msg.type === M.PICKUP_COLLECT && msg.playerId !== this.profile().id) this.opts?.onPickupCollectNet?.(msg);
    ...
    tick(dt) {
      ...
      if (this.isHost && g?.enemies && performance.now() - this.lastEnemySend > 1000 / NET_CONFIG.enemySnapshotHz) {
        ...
        this.client?.send(NetProtocol.MSG.ENEMY_STATE, { enemies:g.enemies.filter(e=>e.netId).map(e=>({ id:e.netId, typeId:e.typeId, boss:e.boss, x:e.pos.x, z:e.pos.z, y:e.rootY || 0, yaw:e.yaw || 0, hp:e.hp, maxHp:e.maxHp, state:e.state })) });
      }
      this.tickRemotes();
    },
    tickRemotes() {
      for (const r of this.remote.values()) {
        ...
        r.avatar.name.textContent = (s.name || r.player.name || 'OPERATIVE') + (s.hp !== undefined ? '  HP ' + Math.max(0, Math.round(s.hp)) : '');
    ...
    onPickupSpawn(pk) { if (this.isCoop && this.isHost && pk?.netId) this.client?.send(NetProtocol.MSG.PICKUP_SPAWN, { pickup:{ id:pk.netId, kind:pk.kind, x:pk.x, z:pk.z, val:pk.val } }); },
