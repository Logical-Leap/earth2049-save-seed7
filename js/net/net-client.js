...
(function (global) {
  class NetClient extends EventTarget {
    constructor(config) { super(); this.config = config || global.NET_CONFIG; this.ws = null; this.roomCode = ''; this.connected = false; this.player = null; this.seq = 0; }
    ...
    connect(roomCode, player) {
      return new Promise((resolve, reject) => {
        ...
        ws.onopen = () => { clearTimeout(timer); this.connected = true; this.emit('open', { roomCode:this.roomCode }); resolve(); };
        ...
        ws.onclose = ev => { this.connected = false; this.emit('close', { code:ev.code, reason:ev.reason }); };
      });
    }
    ...
    close() { if (this.ws) this.ws.close(1000, 'client_leave'); this.ws = null; this.connected = false; }
